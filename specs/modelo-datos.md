...
**Y no es un cuadro sino la lista de los que ha habido**, cada uno con el instante desde el que valió. Un turno sin fila de `paseo` se deriva del cuadro, y con un cuadro único se derivaba del de ahora: cambiar el reparto reescribía hacia atrás los días que nadie llegó a marcar. Cada turno toma el cuadro que gobernaba **cuando se abrió su ventana**, de modo que editar a las seis de la tarde cambia el turno de noche y deja intacto el de la mañana. Guardar añade una versión en lugar de pisar la anterior, salvo que entre las dos no se haya abierto ninguna ventana —los catorce toques de una misma edición son una sola decisión—. El formato anterior, un cuadro suelto, se lee como una versión sin fecha que vale desde siempre, y por eso el cambio no llevó migración. El análisis y las alternativas están en `specs/propuesta-cuadro-con-vigencia.html`.

**Paseo.** Un turno que ha dejado de derivarse del cuadro: fecha, turno —mañana o noche—, persona asignada, persona que lo sacó y cuándo se contestó. La fila aparece cuando alguien marca el turno, cuando se acuerda un cambio para ese día o cuando se contesta que no salió, y desde entonces manda sobre el cuadro. **Que no salió se guarda con la marca de cuándo se contestó y sin persona que lo sacara**: hay respuesta y no hay quien lo hiciera. Es lo único que distingue no haber salido —que se sabe— de no haberlo apuntado, que es no saberlo y no tiene fila ninguna; y no hizo falta columna nueva porque esa combinación no la escribe ninguna otra operación. Es la regla que permite cambiar el reparto sin reescribir el pasado. El identificador es compuesto y determinista —`lio:2026-07-27:manana`—, porque el dispositivo lo necesita antes de haber visto ninguna fila; el par fecha y turno es único.

**TratoDia.** La propuesta sobre un día concreto de una actividad —quién lleva o quién recoge ese martes— cuando se le pide a otro (specs/propuesta-plugins-hojas.html, K1). Misma forma que el trato de un paseo: actividad, fecha, campo (`lleva` o `recoge`), quien propone, a quién se le pide, quién lo tenía y quién lo tendría, y un estado que solo cambia el destinatario; caduca al pasar el día. Mientras está pendiente el día se queda como estaba; aceptar escribe la fila de `evento_dia` que resulta. Quedarse uno con el recado, o soltar el propio sin cargárselo a nadie, no pasa por aquí.

**TratoPaseo.** Una propuesta sobre un turno, que no surte efecto hasta que la acepta su destinatario. Hay dos clases. `cambio` la abre quien tiene el turno y no puede sacarlo. `correccion` la abre quien dice que sacó **el turno de otro que ya venció sin marcar**, y va contra quien lo tenía: no se está cargando con un recado sino escribiendo, un día después, qué pasó con el turno de alguien. Coger un turno ajeno cuya ventana sigue abierta, en cambio, no abre trato ninguno y se escribe en el acto. Guarda quién propone, a quién, a qué asignación se vuelve si la respuesta es que no, y en qué quedó —pendiente, aceptado, rechazado o caducado—. Mientras está pendiente no altera nada, de modo que rechazar es exactamente no haber propuesto. Solo el destinatario la resuelve; el proponente únicamente puede retirarla.
...
**Apunte.** Cuelga de un lugar, con una clase —`llevar`, `hacer`, `ir` o `saber`—, un título, un detalle opcional y una marca `hecho`. La marca es el único estado de todo el módulo y solo la usa `llevar`, que se lee como lista de la compra: «subir a la duna» no se agota al subir, pero la sombrilla o está en el maletero o no está. Lo tachado se queda y baja al final de su grupo. Un apunte de `llevar` no guarda detalle: allí no hay dónde leerlo, y un dato que no se ve es un dato que miente. La clase no lleva `CHECK` en el esquema por lo mismo que no lo lleva ya el comentario: la lista vive en el código, y un `CHECK` de SQLite no se altera. `saber` es el valor por defecto.
