...
**Dentro de un sitio no hay botón flotante: cada grupo lleva su propia fila de escribir al final**, un título y ya está, sin abrir nada. Es lo que sustituye al «+» de dentro —meter cosas rápido es escribir la siguiente línea de la lista que se está mirando, y refinarlas es para después—, y por eso el flotante desaparece en esta altura y solo sigue creando sitios en la lista. En Llevar la fila no lleva más que el título y confirmarlo, porque una lista de la compra no tiene detalle que asociarle; en las demás clases lleva además un lápiz que entrega lo ya escrito, sin perderlo, a la misma hoja de siempre —con la clase puesta—, que es donde se describe, se pide una idea a la IA o se guarda tal cual. Las clases que todavía no tienen nada no rompen la regla de los grupos vacíos: se enseñan como una píldora de borde discontinuo —«+ Ir»—, siempre a la vista, y tocarla es lo único que abre un grupo sin apuntes todavía, con su fila de escribir ya enfocada.
