...
**Rastro de la lectura.** Cada sincronización deja escrito en el propio calendario **cuándo se intentó** y **qué salió**: cuántos `VEVENT` traía el feed, cuántos se entendieron, qué se dio de alta, de baja o de cambio, y el motivo de lo descartado. Hasta ahora lo único que quedaba era `ultima_sincronizacion`, que solo se sella cuando la cosa sale bien; con eso, un feed que se lee correctamente y viene vacío y otro que lleva semanas dando 404 dejan el mismo rastro —una fecha cada vez más vieja— y desde la agenda se ven igual: no hay viaje. Son dos columnas porque contestan a dos preguntas: un intento reciente con resultado de error dice «el proceso corre y falla», y un intento viejo dice «el proceso no está corriendo».

**Componentes anidados.** Un `VEVENT` puede llevar dentro sus alarmas (`VALARM`), y una alarma trae `SUMMARY` y `DESCRIPTION` propios. Solo se leen las propiedades del `VEVENT` en su propio nivel: sin eso, la última lectura gana y el vuelo se importa titulado «Recordatorio» y con el texto de la alarma por notas. No desaparece —que es lo que parece desde la agenda— sino que llega con otro nombre, y así ni se reconoce ni lo traduce `presentarVuelo`.
